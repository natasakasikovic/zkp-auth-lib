use auth::{AuthContext, SecretKey, create_auth_proof, current_unix_timestamp};
...
fn report_auth_sizes() {
    ...
    for i in 0..ITERATIONS {
        ...
        if i == 0 {
            raw_proof_size = bundle.proof.commitment.as_bytes().len() + bundle.proof.response.len();
        }

        let json_proof =
            serde_json::to_vec(&bundle.proof).expect("Proof JSON serialization should succeed");

        let json_bundle =
            serde_json::to_vec(&bundle).expect("Bundle JSON serialization should succeed");

        let bincode_proof =
            bincode::serialize(&bundle.proof).expect("Proof bincode serialization should succeed");

        let bincode_bundle =
            bincode::serialize(&bundle).expect("Bundle bincode serialization should succeed");
    ...
    let avg_json_proof = json_proof_sizes.iter().sum::<usize>() as f64 / ITERATIONS as f64;

    let min_json_bundle = *json_bundle_sizes.iter().min().unwrap();
    let max_json_bundle = *json_bundle_sizes.iter().max().unwrap();
    let avg_json_bundle = json_bundle_sizes.iter().sum::<usize>() as f64 / ITERATIONS as f64;

    let min_bincode_proof = *bincode_proof_sizes.iter().min().unwrap();
    let max_bincode_proof = *bincode_proof_sizes.iter().max().unwrap();
    let avg_bincode_proof = bincode_proof_sizes.iter().sum::<usize>() as f64 / ITERATIONS as f64;

    let min_bincode_bundle = *bincode_bundle_sizes.iter().min().unwrap();
    let max_bincode_bundle = *bincode_bundle_sizes.iter().max().unwrap();
    let avg_bincode_bundle = bincode_bundle_sizes.iter().sum::<usize>() as f64 / ITERATIONS as f64;

    println!();
    println!("=== Authentication size report ===");
    println!("Iterations: {ITERATIONS}");

    println!("Raw SchnorrProof size: {} B", raw_proof_size);

    println!(
        "JSON SchnorrProof: min={} B, avg={:.2} B, max={} B",
        min_json_proof, avg_json_proof, max_json_proof
    );

    println!(
        "JSON AuthProofBundle: min={} B, avg={:.2} B, max={} B",
        min_json_bundle, avg_json_bundle, max_json_bundle
    );

    println!(
        "bincode SchnorrProof: min={} B, avg={:.2} B, max={} B",
        min_bincode_proof, avg_bincode_proof, max_bincode_proof
    );

    println!(
        "bincode AuthProofBundle: min={} B, avg={:.2} B, max={} B",
        min_bincode_bundle, avg_bincode_bundle, max_bincode_bundle
    );
